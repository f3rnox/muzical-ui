...
export default function writeStoredPlaybackPreferences(
  ...
  try {
    ...
    window.localStorage.setItem(PLAYBACK_RATE_STORAGE_KEY, String(prefs.playbackRate))
    window.localStorage.setItem(REMEMBER_VOLUME_STORAGE_KEY, JSON.stringify(prefs.rememberVolume))
    window.localStorage.setItem(VOLUME_STORAGE_KEY, String(prefs.volume))
    window.localStorage.setItem(SEEK_STEP_SMALL_STORAGE_KEY, String(prefs.seekStepSmallSec))
    window.localStorage.setItem(SEEK_STEP_LARGE_STORAGE_KEY, String(prefs.seekStepLargeSec))
    window.localStorage.setItem(AUTO_ADVANCE_ON_END_STORAGE_KEY, JSON.stringify(prefs.autoAdvanceOnEnd))
  } catch {
    /* ignore */
  }
}
