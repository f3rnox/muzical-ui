...
function readPlaybackRate(): number {
  ...
  const v = Number.parseFloat(window.localStorage.getItem(PLAYBACK_RATE_STORAGE_KEY) ?? '')
...
function readVolume(): number {
  ...
  const v = Number.parseFloat(window.localStorage.getItem(VOLUME_STORAGE_KEY) ?? '')
...
export default function readStoredPlaybackPreferences(): PlaybackPreferences {
  ...
  return {
    ...
    rememberVolume: readStoredBoolean(REMEMBER_VOLUME_STORAGE_KEY, defaults.rememberVolume),
    volume: readVolume(),
    seekStepSmallSec: readPositiveInt(SEEK_STEP_SMALL_STORAGE_KEY, defaults.seekStepSmallSec, 120),
    seekStepLargeSec: readPositiveInt(SEEK_STEP_LARGE_STORAGE_KEY, defaults.seekStepLargeSec, 600),
    autoAdvanceOnEnd: readStoredBoolean(AUTO_ADVANCE_ON_END_STORAGE_KEY, defaults.autoAdvanceOnEnd),
  }
}
