...
export function PlaybackPreferencesProvider(props: { children: ReactNode }) {
  ...
  return (
    <PlaybackPreferencesContext.Provider value={value}>{props.children}</PlaybackPreferencesContext.Provider>
  )
}
...
export function usePlaybackPreferences(): PlaybackPreferencesContextValue {
  ...
  if (!ctx) {
    throw new Error('usePlaybackPreferences must be used within PlaybackPreferencesProvider')
  }
  return ctx
}
