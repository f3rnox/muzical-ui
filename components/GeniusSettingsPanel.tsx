...
import readStoredGeniusApiKey from '@/lib/genius/read-stored-genius-api-key'
import writeStoredGeniusApiKey from '@/lib/genius/write-stored-genius-api-key'

export default function GeniusSettingsPanel() {
  const { notifySettingsSaved } = useSettingsSaveNotification()
  const [apiKey, setApiKey] = useState('')

  useEffect(() => {
    setApiKey(readStoredGeniusApiKey())
  }, [])

  const onSave = useCallback(() => {
    writeStoredGeniusApiKey(apiKey)
    notifySettingsSaved('Genius settings saved')
  }, [apiKey, notifySettingsSaved])

  return (
    <div className="flex flex-col gap-8">
      <div>
        <h2 className="text-lg font-semibold tracking-tight text-zinc-900 dark:text-zinc-100">Genius</h2>
        <p className="mt-1 text-sm text-zinc-600 dark:text-zinc-400">
          Provide a Genius API key to enable lyrics fetching. It is stored only in this browser.
        </p>
      </div>
      ...
      <section className="rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-900/40">
        ...
        <label className="mt-4 block">
          <span className="sr-only">Genius API key</span>
          <input
            type="password"
            autoComplete="off"
            spellCheck={false}
