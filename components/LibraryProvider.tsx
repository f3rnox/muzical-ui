...
const DEFAULT_REMEMBER_LAST_QUEUE = true;
...
function safeReadStoredBooleanOrDefault(
  key: string,
...
function safeWriteStoredStringArray(
  key: string,
...
export function LibraryProvider(props: { children: ReactNode }) {
  ...
  const rootHandlesRef = useRef<Map<string, FileSystemDirectoryHandle>>(
    new Map(),
  );
  const rootsMetaRef = useRef<LibraryRootMeta[]>([]);
  const libraryTracksRef = useRef<Track[]>([]);
  const persistCatalogTimerRef = useRef<ReturnType<typeof setTimeout> | null>(
    null,
  );
  const scanDismissTimerRef = useRef<ReturnType<typeof setTimeout> | null>(
    null,
  );
  const favoritesReadyRef = useRef(false);
  const recentBrowseSearchesReadyRef = useRef(false);
  const scanPrefsRef = useRef<LibraryScanPreferences>(
    readStoredLibraryScanPreferences(),
  );
  const logLibraryScanTimingRef = useRef(false);
  const rememberLastQueueRef = useRef(DEFAULT_REMEMBER_LAST_QUEUE);
  ...
  const persistPlaybackTimerRef = useRef<ReturnType<typeof setTimeout> | null>(
    null,
  );
  ...
  const [recentBrowseSearches, setRecentBrowseSearches] = useState<
    RecentBrowseSearch[]
  >([]);
  ...
  const [scanPreferences, setScanPreferencesState] =
    useState<LibraryScanPreferences>(readStoredLibraryScanPreferences);
  const [rememberLastQueue, setRememberLastQueueState] = useState(
    DEFAULT_REMEMBER_LAST_QUEUE,
  );
  const [playbackRestore, setPlaybackRestore] =
    useState<PlaybackRestore | null>(null);
  const [playNowRequest, setPlayNowRequest] = useState<PlayNowRequest | null>(
    null,
  );
  ...
  const [relatedSongsSeedTrack, setRelatedSongsSeedTrack] =
  ...
  const endYoutubePrefetch = useCallback((videoCount: number) => {
    const n = Math.max(0, Math.floor(videoCount));
    youtubePrefetchVideosRef.current = Math.max(
      0,
      youtubePrefetchVideosRef.current - n,
    );
    setYoutubePrefetchVideoCount(youtubePrefetchVideosRef.current);
  ...
  useEffect(() => {
    void Promise.resolve().then(() => {
      const on = safeReadStoredBooleanOrDefault(
        STORAGE_REMEMBER_LAST_QUEUE,
        DEFAULT_REMEMBER_LAST_QUEUE,
      );
  ...
  useEffect(() => {
    ...
    writeStoredRecentBrowseSearches(
      recentBrowseSearches.slice(0, RECENT_BROWSE_SEARCHES_LIMIT),
    );
  ...
  const flushPersistPlaybackNow = useCallback((): void => {
    ...
    const activeQueueIndex = activeId
      ? q.findIndex((row) => row.queueId === activeId)
      : 0;
    const activeRow =
      activeQueueIndex >= 0 && activeQueueIndex < q.length
        ? q[activeQueueIndex]
        : q[0];
    writeStoredPlaybackSnapshot({
      ...
      activeQueueIndex: activeRow ? Math.max(0, activeQueueIndex) : null,
      positionSec: playbackReportRef.current.positionSec,
      wasPlaying: playbackReportRef.current.isPlaying,
    });
  }, [queue]);
  ...
  const runScan = useCallback(
    async (withUserActivation = false): Promise<void> => {
      ...
      const merged = mergeScannedTracksWithSavedLibrary(
        next,
        libraryTracksRef.current,
      );
      ...
      if (
        ...
        setScanError(
          firstError ? `${firstError} ${DISK_ACCESS_HINT}` : DISK_ACCESS_HINT,
        );
      }
    },
    [performScan],
  ...
  const patchAlbumMetadataByKey = useCallback(
    (albumKey: string, fields: AlbumMetadataFields): string | null => {
      ...
      const matchesKey = (t: Track): boolean =>
        albumCompositeKey(t.album, t.artist) === key;
      ...
      setQueue((prev) =>
        prev.map((q) => ({ ...q, track: patchTrack(q.track) })),
      );
      setDetailsTrack((prev) =>
        prev && matchesKey(prev) ? patchTrack(prev) : prev,
      );
  ...
  const patchArtistMetadataByKey = useCallback(
    (artistName: string, fields: ArtistMetadataFields): string | null => {
      ...
      const matchesArtist = (t: Track): boolean =>
        artistDisplayName(t.artist) === oldName;
      const toPatch = tracksMatchingArtistName(
        libraryTracksRef.current,
        oldName,
      );
      if (toPatch.length === 0) return null;
      ...
      setDetailsTrack((prev) =>
        prev && matchesArtist(prev) ? patchTrack(prev) : prev,
      );
  ...
  useEffect(() => {
    ...
    const targets = collectYoutubePrefetchTargets(
      queue.map((row) => row.track),
    ).slice(0, limit);
  ...
  const reorderQueueItems = useCallback(
    (fromIndex: number, toIndex: number) => {
      ...
      setQueue((prev) => {
        if (fi < 0 || ti < 0 || fi >= prev.length || ti >= prev.length)
          return prev;
  ...
  const recordTrackPlaybackProgress = useCallback(
    (trackId: string, listenedSec: number) => {
      const id = trackId.trim();
  ...
  const recordRecentBrowseSearch = useCallback(
    (source: BrowseSearchSource, query: string) => {
  ...
  const renamePlaylist = useCallback(
    (playlistId: string, name: string): boolean => {
      const cleanName = normalizePlaylistName(name);
  ...
  const removeTracksFromPlaylist = useCallback(
    (playlistId: string, trackIds: readonly string[]) => {
      ...
      setPlaylists((prev) =>
        prev.map((p) =>
          p.id === playlistId ? removeTrackIdsFromPlaylist(p, trackIds) : p,
        ),
      );
    },
    [],
  ...
  const reorderPlaylistTracks = useCallback(
    (playlistId: string, fromIndex: number, toIndex: number) => {
      setPlaylists((prev) =>
        prev.map((p) =>
          p.id === playlistId
            ? reorderPlaylistTrackIds(p, fromIndex, toIndex)
            : p,
        ),
      );
    },
  ...
  const reportPlayback = useCallback(
    (
      ...
      playbackReportRef.current = {
        activeQueueId,
        positionSec,
        isPlaying: isPlaying ?? playbackReportRef.current.isPlaying,
      };
      persistPlaybackDebounced();
    },
    [persistPlaybackDebounced],
  ...
  const downloadTrack = useCallback(
    (track: Track): void => {
      void downloadTrackAudio(track, resolveFileForTrack).catch(
        (error: unknown) => {
          const message =
  ...
  const favoriteSongSet = useMemo(
    () => new Set(favoriteSongIds),
    [favoriteSongIds],
  );
  const favoriteArtistSet = useMemo(
    () => new Set(favoriteArtistNames),
    [favoriteArtistNames],
  );
  const favoriteAlbumSet = useMemo(
    () => new Set(favoriteAlbumKeys),
    [favoriteAlbumKeys],
  );
  ...
  const removeLibraryFolder = useCallback(
    async (id: string): Promise<void> => {
      ...
      try {
        ...
        setScanError(
          e instanceof Error ? e.message : "Could not remove library folder",
        );
      }
    },
    [runScan],
  ...
  useEffect(() => {
    ...
    void (async (): Promise<void> => {
      ...
      try {
        ...
        if (disposed) return;
        const mayRescanOnLoad =
          meta.length > 0 && (await everyHandleHasGrantedReadAccess(map));
        ...
        if (mayRescanOnLoad && shouldAutoRescan && !disposed) {
          ...
          const needsGesture =
            next.length === 0 && meta.length > 0 && failedRootCount > 0;
          if (needsGesture) {
            if (cachedApplied) {
              setScanError(null);
            } else {
              catalogTracks = mergeScannedTracksWithSavedLibrary(
                next,
                savedTracksFromCache,
              );
              setLibraryTracks(catalogTracks);
              try {
                await idbPutCatalog(
  ...
  return (
    <LibraryContext.Provider value={value}>
      {props.children}
      <LibraryScanNotification
        progress={scanProgress}
        onDismiss={dismissScanNotification}
      />
      <TrackDetailsDialog track={detailsTrack} onClose={closeTrackDetails} />
      <RelatedSongsDialog
        seedTrack={relatedSongsSeedTrack}
        onClose={closeRelatedSongs}
      />
      {addToPlaylistTarget ? (
        <AddToPlaylistDialog
